...
In **hq-to-video**’s backend, prefer **[zod-type-provider.md](zod-type-provider.md)** (Zod 4 + `fastify-type-provider-zod`) for route schemas and OpenAPI. This document remains useful for generic JSON Schema, AJV options, and TypeBox-oriented examples.
